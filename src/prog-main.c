...
/* struct file_options and MACROs from config_file.[ch] */
struct file_options fileoptions[] = {
	...
	{ FILEOPT_LINE(ipam_comment2, struct st_options, TYPE_STRING),
		"IPAM CSV header field describing comment" },
	{ FILEOPT_LINE(ipam_delim, struct st_options, TYPE_STRING),
		"IPAM CSV delimitor" },
	{ FILEOPT_LINE(ipam_comment_delim, struct st_options, TYPE_CHAR),
		"IPAM CSV comment delimitor" },
	{ FILEOPT_LINE(ipam_comment_delim_escape, struct st_options, TYPE_CHAR),
		"IPAM CSV comment delimitor escape" },
...
static struct st_options nof = {
	...
	.ipam_delim	= ",",
	.ipam_ea 	= "comment",
	.ipam_comment_delim = '\0',
	.ipam_comment_delim_escape = '\0',
...
static int option_fmt(int argc, char **argv, void *st_options)
{
	...
	/* sizeof(xxx_output_fmt) are the same, no need to check for overrun */
	strcpy(nof->output_fmt, argv[1]);
	strcpy(nof->bgp_output_fmt, argv[1]);
	strcpy(nof->ipam_output_fmt, argv[1]);
	strcpy(nof->scanf_output_fmt, argv[1]);
