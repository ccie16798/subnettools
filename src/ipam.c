...
int load_ipam(char  *name, struct ipam_file *sf, struct st_options *nof)
{
	...
	if (csv_field == NULL)
		return -1;
	init_csv_file(&cf, name, csv_field, ea_nr + 4, nof->ipam_delim, &simple_strtok_r);
	...
	register_csv_field(&cf, s, 1, 0, ipam_prefix_handle);
	s = (nof->ipam_mask[0] ? nof->ipam_mask : "netmask_dec");
	register_csv_field(&cf, s, 1, 0, ipam_mask_handle);
	...
	while (s) {
		...
		register_csv_field(&cf, s, 0, 0, ipam_ea_handle);
		s = strtok(NULL, ",");
	}
	if (i == 0) {
